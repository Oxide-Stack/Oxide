...
#   1) Bump VERSION, update changelogs, run tools/scripts/version_sync.{ps1,sh}, commit + push to develop
...
case "$COMMAND" in
  ...
  release)
    ...
    # Preflight: enforce VERSION and changelog entries before tagging.
    repo_root="$(git rev-parse --show-toplevel 2>/dev/null || true)"
    [[ -n "$repo_root" ]] || die "Failed to detect repository root."
    version_path="$repo_root/VERSION"
    [[ -f "$version_path" ]] || die "VERSION file not found at $version_path"
    repo_version="$(cat "$version_path" | tr -d '\r\n' | xargs)"
    [[ -n "$repo_version" ]] || die "VERSION file is empty."
    if [[ "$repo_version" != "$version" ]]; then
      die "Release version '$version' does not match VERSION '$repo_version'. Run version sync and update changelogs before tagging."
    fi

    changelog_files=(
      "CHANGELOG.md"
      "flutter/oxide_annotations/CHANGELOG.md"
      "flutter/oxide_generator/CHANGELOG.md"
      "flutter/oxide_runtime/CHANGELOG.md"
    )
    missing_changelogs=()
    for rel in "${changelog_files[@]}"; do
      path="$repo_root/$rel"
      if [[ ! -f "$path" ]]; then
        missing_changelogs+=("$rel (missing file)")
        continue
      fi
      if ! grep -qE "^##[[:space:]]+$repo_version[[:space:]]*$" "$path"; then
        missing_changelogs+=("$rel (missing '## $repo_version' entry)")
      fi
    done
    if [[ "${#missing_changelogs[@]}" -gt 0 ]]; then
      echo "Changelog check failed for version $repo_version. Add entries before tagging:" >&2
      printf '  - %s\n' "${missing_changelogs[@]}" >&2
      exit 2
    fi
    tag="v$version"
