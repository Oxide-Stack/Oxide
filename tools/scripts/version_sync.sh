...
for file in "${cargo_tomls[@]}"; do
  update_file_regex "$file" "cargo dep sync" \
    "s/(oxide_(core|generator_rs))[[:space:]]*=[[:space:]]*\\{[[:space:]]*version[[:space:]]*=[[:space:]]*\"[0-9]+\\.[0-9]+\\.[0-9]+\"/\\1 = { version = \"$VERSION\"/g" || exit $?
done
...
docs=(
  ...
  "$ROOT_DIR/rust/oxide_generator_rs/README.md"
...
for file in "${docs[@]}"; do
  update_file_regex "$file" "doc sync" \
    "s/^(Status:[[:space:]]*)\`[^\`]*\`([.][[:space:]]*)$/\\1\`$VERSION\`\\2/; \
     ...
     s/(oxide_(core|generator_rs))[[:space:]]*=[[:space:]]*\"[0-9]+\\.[0-9]+\\.[0-9]+\"/\\1 = \"$VERSION\"/g; \
     s/(oxide_(core|generator_rs))[[:space:]]*=[[:space:]]*\\{[[:space:]]*version[[:space:]]*=[[:space:]]*\"[0-9]+\\.[0-9]+\\.[0-9]+\"/\\1 = { version = \"$VERSION\"/g; \
     s/^(##[[:space:]]+)[0-9]+\\.[0-9]+\\.[0-9]+[[:space:]]*$/\\1$VERSION/" || exit $?
done

echo "Reminder: update changelogs for version $VERSION before releasing:"
echo "- CHANGELOG.md"
echo "- flutter/oxide_annotations/CHANGELOG.md"
echo "- flutter/oxide_generator/CHANGELOG.md"
echo "- flutter/oxide_runtime/CHANGELOG.md"
