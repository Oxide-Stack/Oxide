...
switch ($Cmd) {
  ...
  "release" {
    ...
    if ($LASTEXITCODE -ne 0 -or [string]::IsNullOrWhiteSpace($repoRoot)) {
      throw "Failed to detect repository root."
    }
    $versionPath = Join-Path $repoRoot "VERSION"
    if (-not (Test-Path $versionPath)) {
      throw "VERSION file not found at $versionPath"
    }
    $repoVersion = (Get-Content $versionPath -Raw).Trim()
    if ([string]::IsNullOrWhiteSpace($repoVersion)) {
      throw "VERSION file is empty."
    }
    if ($repoVersion -ne $version) {
      throw ("Release version '{0}' does not match VERSION '{1}'. Run version sync and update changelogs before tagging." -f $version, $repoVersion)
    }
    ...
    foreach ($rel in $changelogFiles) {
      $path = Join-Path $repoRoot $rel
      if (-not (Test-Path $path)) {
        $missing.Add(("{0} (missing file)" -f $rel))
        continue
      }
      $pattern = ('^##\s+' + [regex]::Escape($repoVersion) + '\s*$')
      if (-not (Select-String -Path $path -Pattern $pattern -Quiet)) {
        $missing.Add(("{0} (missing '## {1}' entry)" -f $rel, $repoVersion))
      }
    }
    if ($missing.Count -gt 0) {
      $message = "Changelog check failed for version $repoVersion. Add entries before tagging:`n- " + ($missing -join "`n- ")
      throw $message
    }
