...
try {
  $validCrates = @("oxide_core", "oxide_generator_rs")
  if ($Crate -and ($validCrates -notcontains $Crate)) {
    throw "Unknown crate: $Crate"
  }
  ...
  if (-not $Crate) {
  ...
  } elseif ($Crate -eq "oxide_core") {
    Invoke-QACommand "cargo" @("test", "-p", "oxide_core")
    Invoke-QACommand "cargo" @("test", "-p", "oxide_core", "--features", "navigation-binding,isolated-channels")
    ...
    Invoke-QACommand "cargo" @("test", "-p", "oxide_generator_rs")
    Invoke-QACommand "cargo" @("test", "-p", "oxide_generator_rs", "--test", "compile")
  }

  if (-not $SkipCoverage -and ($env:QA_SKIP_COVERAGE -ne "1") -and (-not $Crate -or $Crate -eq "oxide_core")) {
    ...
    } elseif ($env:QA_REQUIRE_COVERAGE -eq "1") {
