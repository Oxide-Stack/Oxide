...
foreach ($cargoPath in $cargoTomlPaths) {
  $change = Update-FileText $cargoPath {
    ...
    [regex]::Replace(
      $content,
      '(?<name>oxide_(core|generator_rs))\s*=\s*\{\s*version\s*=\s*"[0-9]+\.[0-9]+\.[0-9]+"',
...
$docsToUpdate = @(
  ...
  @{ Path = "rust\oxide_generator_rs\README.md"; Label = "oxide_generator_rs README" },
...
foreach ($entry in $docsToUpdate) {
  ...
  $change = Update-FileText $path {
    ...
    $updated = [regex]::Replace(
      $updated,
      '(?<name>oxide_(core|generator_rs))\s*=\s*"[0-9]+\.[0-9]+\.[0-9]+"',
...
Write-Host "Reminder: update changelogs for version $version before releasing:"
Write-Host "- CHANGELOG.md"
Write-Host "- flutter/oxide_annotations/CHANGELOG.md"
Write-Host "- flutter/oxide_generator/CHANGELOG.md"
Write-Host "- flutter/oxide_runtime/CHANGELOG.md"
