...
Future<RustChannelMetadata> readRustChannelMetadata() async {
  ...
  if (!dir.existsSync()) {
    return RustChannelMetadata(events: const [], callbacks: const []);
  }
  ...
  for (final file in files) {
    ...
    // capture the crate name from metadata if present. the JSON files use the
    // same `crate_name` key that route metadata uses. we only record the first
    // appearance; it doesn't matter if other files use a different value because
    // the crate name is only needed for naming the init helper.
    ...
    if (rawCallbacks is List) {
      for (final raw in rawCallbacks) {
        ...
        if (name is String && req is String && resp is String) {
          callbacks.add(
            RustCallbackMeta(name: name, requestType: req, responseType: resp),
          );
        }
      }
    }
...
String generateOxideStackSource({RustChannelMetadata? channels}) {
  ...
  if (hasChannels) {
    ...
    buf.writeln(
      "import '../src/rust/api/isolated_channels_bridge.dart' as _ch;",
    );
  ...
  buf.writeln(
    '  // always perform core init but postpone actual navigation start',
  );
