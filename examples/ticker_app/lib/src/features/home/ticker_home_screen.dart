          ...
          child: AnimatedBuilder(
            ...
            builder: (context, _) {
              ...
              return _TickSection(
                ...
                lastConfirmed: state.tick.lastConfirmed,
        ...
        Expanded(
          child: Builder(
            builder: (context) {
              ...
              if (controlView.isLoading) return const _LoadingView();
              if (controlView.error != null) return _ErrorView(controlView.error);
              if (state == null) return const Center(child: Text('No state'));
              return _ControlSection(
                title: 'Control (slice: control)',
                isRunning: state.control.isRunning,
        ...
        Expanded(
          child: Builder(
            builder: (context) {
              ...
              if (tickView.isLoading) return const _LoadingView();
              if (tickView.error != null) return _ErrorView(tickView.error);
              if (state == null) return const Center(child: Text('No state'));
              return _TickSection(
                title: 'Ticks (slice: tick)',
                ticks: state.tick.ticks.toInt(),
...
class _ControlSection extends StatefulWidget {
  ...
  final Future<void> Function(int intervalMs) onStart;
        ...
        children: [
          ...
          Row(
            children: [
              ...
              FilledButton(
                onPressed: _inFlight ? null : () => _run(() => widget.onStart(_readIntervalMs())),
                child: const Text('Start'),
              ),
              const SizedBox(width: 8),
              OutlinedButton(
                onPressed: _inFlight ? null : () => _run(widget.onStop),
...
class _TickSection extends StatefulWidget {
  const _TickSection({
    required this.title,
    required this.ticks,
  ...
  @override
  State<_TickSection> createState() => _TickSectionState();
}

class _TickSectionState extends State<_TickSection> {
  bool _inFlight = false;
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          Text(widget.title, style: Theme.of(context).textTheme.titleMedium),
          const SizedBox(height: 12),
          Text('Ticks: ${widget.ticks}'),
          const SizedBox(height: 8),
          Text('Last tick source: ${widget.lastTickSource}'),
          const SizedBox(height: 8),
          ...
          Wrap(
            ...
            children: [
              FilledButton(
                onPressed: _inFlight ? null : () => _run(widget.onManualTick),
                child: const Text('Manual tick'),
              ),
              OutlinedButton(
                onPressed: _inFlight ? null : () => _run(widget.onSideEffectTick),
                child: const Text('Side-effect tick'),
              ),
              TextButton(
                onPressed: _inFlight ? null : () => _run(widget.onReset),
