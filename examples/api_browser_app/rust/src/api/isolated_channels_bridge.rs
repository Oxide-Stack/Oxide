...
use crate::isolated_channels_demo::{
    ApiBrowserDemoDialog, ApiBrowserDemoDialogResponse, ApiBrowserDemoDuplex, ApiBrowserDemoEvent,
    ApiBrowserDemoIn, ApiBrowserDemoOut, ApiBrowserDemoEvents, install_duplex_incoming_handler,
    last_incoming_text,
};
...
#[derive(Clone, Debug)]
pub struct ApiBrowserDemoDialogPendingRequest {
    /// Request identifier that must be echoed back when responding.
    pub id: u64,
    /// Typed request payload.
    pub request: crate::isolated_channels_demo::ApiBrowserDemoDialogRequest,
}
...
pub fn emit_api_browser_demo_notification(message: String) {
    ApiBrowserDemoEvents::notify(message);
}

#[flutter_rust_bridge::frb]
/// Streams demo events (Rust → Dart).
pub async fn api_browser_demo_events_stream(
    sink: crate::frb_generated::StreamSink<ApiBrowserDemoEvent>,
) {
    crate::isolated_channels_demo::__oxide_isolated_events_api_browser_demo_events::frb::oxide_events_stream(sink).await
}

#[flutter_rust_bridge::frb]
/// Streams callback requests (Rust → Dart) for the demo dialog service.
pub async fn api_browser_demo_dialog_requests_stream(
    sink: crate::frb_generated::StreamSink<ApiBrowserDemoDialogPendingRequest>,
) {
    loop {
        let Some((id, request)) = crate::isolated_channels_demo::__oxide_isolated_callback_api_browser_demo_dialog::runtime()
            .recv_request()
            .await
        else {
            break;
        };
        let _ = sink.add(ApiBrowserDemoDialogPendingRequest { id, request });
    }
}
...
pub async fn api_browser_demo_dialog_confirm_via_frb_callback(
    title: String,
...
pub async fn api_browser_demo_duplex_outgoing_stream(
    sink: crate::frb_generated::StreamSink<ApiBrowserDemoOut>,
) {
    crate::isolated_channels_demo::__oxide_isolated_duplex_api_browser_demo_duplex::frb::oxide_outgoing_stream(sink).await
}

#[flutter_rust_bridge::frb]
/// Sends a duplex outgoing message from Rust to Dart.
pub fn api_browser_demo_duplex_send(text: String) {
    ApiBrowserDemoDuplex::send(ApiBrowserDemoOut::Send { text });
}
...
pub fn api_browser_demo_last_incoming_text() -> Option<String> {
    last_incoming_text()
}
