...
fn log_stream_captures_all_levels_and_initializes_once() {
    let mut rx = oxide_core::ffi::logger::setup_log_stream().expect("first setup should initialize sink");
    ...
    assert!(entries.iter().any(|(level, target, message)| {
        level == "trace" && target.contains("oxide_core::tests") && message.contains("trace message")
    }));
    assert!(entries.iter().any(|(level, _, message)| {
        level == "debug" && message.contains("debug message")
    }));
    assert!(entries.iter().any(|(level, _, message)| {
        level == "info" && message.contains("info message")
    }));
    assert!(entries.iter().any(|(level, _, message)| {
        level == "warn" && message.contains("warn message")
    }));
    assert!(entries.iter().any(|(level, _, message)| {
        level == "error" && message.contains("error message")
    }));
