...
This repository includes a runnable benchmark app that compares Dart-only state updates with Rust/Oxide-backed state updates across several Flutter state management integrations.

For the runnable benchmark harness, see [examples/benchmark_app](../examples/benchmark_app).

## What Is Measured

Each recorded sample is end-to-end wall time for one benchmark action run inside a Flutter app:

- Dispatch work for the selected variant
- State update work for the selected workload
- Waiting for a frame boundary (`endOfFrame`)

That makes the results useful for answering: how does this approach behave inside a Flutter UI loop?
...
High intensity batches more work into a single action run, which tends to amortize fixed overheads such as dispatch, framework glue, and Rust↔Dart boundary costs.
...
- Compare medians when you care about typical responsiveness, and p95 when you care about tail latency and jank risk.
- Low-intensity results are more sensitive to integration overhead (framework glue, dispatching, and Rust↔Dart boundaries). If the workload is tiny, overhead dominates.
- High-intensity results are more sensitive to raw compute cost (CPU and parsing). If the workload is heavy, batching and native performance can dominate.

## Where Oxide Fits vs Pure Dart

Oxide is a good replacement for pure Dart in parts of your app that are both performance-sensitive and correctness-sensitive:
